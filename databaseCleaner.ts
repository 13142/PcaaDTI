...
let restoreQuery;
export function init(connectionIn: mysql.Connection) {
    connection = connectionIn;
}

export function takeSnapshot(tableToTake: String, done: Function) {
    connection.connect(err => {
        ...
        connection.query("SELECT * FROM `" + tableToTake + "`;", (err2, res: any[]) => {
            if (err2) {
                done(err2);
                return;
            }
...
export function restore(done: Function, endConnection = true) {
