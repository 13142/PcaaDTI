...
$("#removalConfirm").on("show.bs.modal", function (e) {
    ...
    if ($(e.relatedTarget).hasClass("massEditRemove")) {
        latestTargetInfo.id = massEditCheckboxIds(latestTargetInfo.table, cur => {
...
function checkForErrors(inputToCheck, sameMode = false) {
...
function otherEntryRemovalCheck(allIDs, relevantTable) {
...
$("#idSearchBox").on("input", e => {
    ...
    let detailArray = databaseDesc[searchID.tableID][0];
