...
import * as Q from "q"; //Switch to bluebird
...
let latestQuery;
app.get("/login", (req: express.Request & { session: any }, res: express.Response) => {
...
app.post("/login", (req: express.Request & { session: any } & bodyParser.ParsedAsUrlencoded, res: express.Response) => {
...
app.get("/logout",
    (req: express.Request & { session: (session.Store | any) }, res: express.Response) => {
        if (!(req.session && req.session.user)) {
...
app.get("/", (req: express.Request & { session: any }, res: express.Response) => {
    if (req.session && req.session.user) {
        res.redirect(302, "/main");
        return;
    }
...
function renderMainPage(res: express.Response, username: string, isAdmin: boolean, testingMode: boolean) {
    Q.all([
        ...
        const committeeDetails: Array<any> = res1[8][0];
        const activityDetails: Array<any> = res1[10][0];
...
app.post("/databaseSubmitTesting", (req: express.Request & { session: (session.Store | any) } & bodyParser.ParsedAsJson, res: express.Response) => {
...
app.get("/testingMain", (req: express.Request & { session: (session.Store | any) }, res: express.Response) => {
    if (!(req.session && req.session.user && req.session.user.permissions === "Administrator")) {
...
app.get("/main", (req: express.Request & { session: (session.Store | any) }, res: express.Response) => {
    if (!(req.session && req.session.user)) {
        res.redirect(302, "/login"); //403 doesnt work properly :\
        return;
    }
...
app.post("/newAccount", (req: express.Request & bodyParser.ParsedAsJson & { session: (session.Store | any) }, res: express.Response) => {
    if (!(req.session && req.session.user && req.session.user.permissions === "Administrator")) {
        res.status(403);
        res.end("ERROR");
        return;
    ...
    for (let i = 0; i < req.body.length; i++) {
        if (req.body[i].name.toLowerCase() === "password") {
            ...
            const hash: Uint8Array = sodium.crypto_pwhash_str(
                passwordBuffer,
...
app.post("/removeAccount",
    (req: express.Request & bodyParser.ParsedAsJson & { session: (session.Store | any) }, res: express.Response) => {
        if (!(req.session && req.session.user && req.session.user.permissions === "Administrator")) {
            res.status(403);
            res.end("ERROR");
            return;
...
function changeUserDetails(session, body, specilizedFunc: Function, callback: Function) {
    const defered = Q.defer();
...
app.post("/permissionChange",
    (req: express.Request & bodyParser.ParsedAsJson & { session: (session.Store | any) }, res: express.Response) => {
        ...
        changeUserDetails(req.session, req.body,
            (session, body, result, callback) => {
...
app.post("/main/submitNewTable",
    (req: bodyParser.ParsedAsJson & express.Request & { session: (session.Store | any) }, res: express.Response) => {
        if (!(req.session && req.session.user)) {
            res.redirect(302, "/login"); //403 doesnt work properly :\
            return;
        }
...
app.post("/main/uploadExcel", upload.single("excelVenue"), (req: multer.Request & express.Request & { session: (session.Store | any) }, res: express.Response) => {
    if (!(req.session && req.session.user)) {
        res.redirect(302, "/login"); //403 doesnt work properly :\
        return;
    }
...
app.get("/main/requestPdf", (req: express.Request & { session: (session.Store | any) }, res: express.Response) => {
    if (!(req.session && req.session.user)) {
        res.redirect(302, "/login"); //403 doesnt work properly :\
        return;
    }
...
function fixEnumTypes(descArray: Array<any>) {
    const typeDetails = [];
